...
def assign_tiers(df_input, tier1_size, tier2_size):
    """Assign tiers based on specified sizes"""
    df_temp = df_input.copy()
    df_temp['tier'] = 0
    
    for year in df_temp['year'].unique():
        year_mask = df_temp['year'] == year
        year_data = df_temp[year_mask].copy()
        
        # Start with direct qualifiers in Tier 1
        direct_qual_mask = year_mask & (df_temp['direct_qualifier_10'] == 1)
        df_temp.loc[direct_qual_mask, 'tier'] = 1
...
def evaluate_tier_sizes(tier1_size, tier2_size):
    """Evaluate a specific tier size configuration"""
...
print("Optimizing Tier Sizes")
print("=" * 80)
print("Testing different configurations of Tier 1 and Tier 2 sizes")
print("(Tier 3 gets all remaining countries)\n")

# Test various tier size combinations
test_configs = []

for tier1 in [6, 8, 10, 12, 14]:
    for tier2 in [6, 8, 10, 12, 14]:
        test_configs.append((tier1, tier2))

print(f"Testing {len(test_configs)} configurations...\n")

results = []
for tier1, tier2 in test_configs:
    result = evaluate_tier_sizes(tier1, tier2)
    results.append(result)
    print(f"  T1={tier1:2d}, T2={tier2:2d} -> Std Dev={result['std_dev']:6.1f}, Range={result['range']:6.1f}")

# Sort by std_dev (lower is better)
results.sort(key=lambda x: x['std_dev'])

print(f"\n{'='*80}")
print("TOP 10 MOST BALANCED TIER SIZE CONFIGURATIONS")
print(f"{'='*80}\n")

for i, r in enumerate(results[:10]):
    print(f"#{i+1}: Tier 1={r['tier1_size']}, Tier 2={r['tier2_size']}")
    print(f"    Std Dev = {r['std_dev']:.1f}, Range = {r['range']:.1f}")
    print(f"    Average tier sizes per year: T1={r['avg_tier_sizes'].get(1, 0):.1f}, T2={r['avg_tier_sizes'].get(2, 0):.1f}, T3={r['avg_tier_sizes'].get(3, 0):.1f}")
